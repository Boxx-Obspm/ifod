...
%     interp1(TimeList0, coord0(:,3), epochs(3), 'linear')];
  refLoc = - refState(1:3) + interp1(TimeList0, coord0, epochs(3), method);
...
%                )./((epochs(3)-epochs(2))*86400.); % km/s^-2
  accEstimate = (interp1(TimeList0, vel0, epochs(3), method) ...
                 - interp1(TimeList0, vel0, epochs(2), method)) ...
                ./((epochs(3)-epochs(2))*86400.); % km/s^-2
...
%                  accEstimate./fa]; <==== gros doute là-dessus!!!
        pState=[X(7:9)./fx; refState(4:6)'./fv; accEstimate'./fa];
        pSigma=eye(9);
%         isInit=false;
  end
